...
void orte_info_register_types(opal_pointer_array_t *mca_types)
{
    ...
    opal_pointer_array_add(mca_types, "urm"); //
    opal_pointer_array_add(mca_types, "test"); //jimmy
                                  ...
                                  opal_pointer_array_t *component_map)
{
    ...
    //==================jimmy======test===============
    if (ORTE_SUCCESS != (rc = orte_test_base_open()) &&
           ORTE_ERR_BAD_PARAM != rc) {
           str = "test_base_open";
           goto error;
       }
       map = OBJ_NEW(opal_info_component_map_t);
       map->type = strdup("test");
       map->components = &orte_test_base.components_available;
       opal_pointer_array_add(component_map, map);
       if (ORTE_ERR_BAD_PARAM == rc)  {
           str = "test";
           goto breakout;
    ...
    //=======================================
       ...
       //==================jimmy======urm===============
          if (ORTE_SUCCESS != (rc = orte_urm_base_open()) &&
                 ORTE_ERR_BAD_PARAM != rc) {
                 str = "urm_base_open";
                 goto error;
             }
             map = OBJ_NEW(opal_info_component_map_t);
             map->type = strdup("urm");
             map->components = &orte_urm_base.components_available;
             opal_pointer_array_add(component_map, map);
             if (ORTE_ERR_BAD_PARAM == rc)  {
                 str = "urm";
                 goto breakout;
...
void orte_info_close_components(void)
{
    ...
    (void) orte_urm_base_close();//
    (void) orte_test_base_close();//jimmy
